...
user-invocable: true
when-to-use: Use when the task matches this skill. Also /loop-on-ci.
argument-hint: /loop-on-ci task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Monitor PR checks and fix failures until green
  host: grok-build
