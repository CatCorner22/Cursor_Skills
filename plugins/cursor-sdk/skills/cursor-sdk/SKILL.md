---
name: cursor-sdk
description: 'Guide users building apps, scripts, CI pipelines, or automations that drive Grok or the xAI API from code. Use when they mention the xAI SDK, Grok API, `XAI_API_KEY`, `https://api.x.ai`, or (legacy) `@cursor/sdk`. Scope boundary: driving a model FROM code. For building a user-facing agent app, use `build-agents` / `eve` instead.'
disable-model-invocation: true
user-invocable: true
when-to-use: 'Trigger on: xAI SDK; Grok API; XAI_API_KEY; api.x.ai; @cursor/sdk. Also /cursor-sdk.'
argument-hint: /cursor-sdk task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Drive the xAI / Grok API from code
  host: grok-build
  ported_from: Cursor_Skills
---
# Programmatic agents (xAI / Grok API)

Prefer the **xAI API** and official SDKs when the user wants to drive Grok from a script, CI job, or backend.

| Host | Start here |
|---|---|
| xAI API | `https://api.x.ai/v1` chat/completions; header `Authorization: Bearer $XAI_API_KEY` |
| Python / TS SDK | Official xAI SDK when the user has it; otherwise OpenAI-compatible client pointed at `api.x.ai` |
| Grok Build CLI | `grok -p "…"` headless; pin a skill with `/skill-name` in the prompt |
| `@cursor/sdk` | Only if the user **names** Cursor's TypeScript SDK. The rest of this file is that legacy surface. |

When the user names `@cursor/sdk`, `Agent.create`, or `CursorAgentError`, keep going with the Cursor SDK sections below. Otherwise design against xAI + Grok Build and do not mention Cursor.
...
If you're seeing 401s, the usual suspects are: key pasted with surrounding whitespace, key minted against a different environment, or the key belongs to a user without repo access for a cloud run. Full troubleshooting: [`references/auth.md`](references/auth.md).
