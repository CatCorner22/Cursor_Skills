---
name: cursor-sdk
description: Guide users building apps, scripts, CI, or automations on the Cursor TypeScript SDK (`@cursor/sdk`). Use when the user mentions `@cursor/sdk`, `Agent.create`, `Agent.prompt`, `Agent.resume`, `run.stream`, or running Cursor agents from a script, GitHub Action, or backend. Scope boundary: driving Cursor's own agent from code only — user-facing agent apps go to `build-agents` / `eve`. This skill plus its references are the source of truth for the package.
---
# Cursor SDK
