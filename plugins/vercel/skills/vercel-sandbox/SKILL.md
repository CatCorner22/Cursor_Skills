...
user-invocable: true
when-to-use: 'Trigger on: @vercel/sandbox; sandbox; code sandbox; vercel sandbox; isolated environment; sandboxed execution; isolated; isolation; untrusted; safely. Also /vercel-sandbox.'
argument-hint: /vercel-sandbox task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Vercel Sandbox + agent-browser guidance — run headless Chrome in Firecracker microVMs for screenshots, accessibility snapshots, and…
  host: grok-build
