...
user-invocable: true
when-to-use: Use when configuring the Next.js bundler, optimizing HMR, debugging build issues, or understanding the Turbopack vs Webpack differences.
argument-hint: /turbopack task
paths:
- next.config.*
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Turbopack expert guidance
  host: grok-build
