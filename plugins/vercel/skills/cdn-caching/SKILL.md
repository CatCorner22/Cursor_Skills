...
user-invocable: true
when-to-use: 'Trigger on: cache hit rate; isr cost; isr read units; isr write units; stale content; x-vercel-cache; cache reason; cacheReason; x-vercel-cache-reason; ppr state. Also /cdn-caching.'
argument-hint: /cdn-caching task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Debug Vercel CDN caching — cache hit rate, stale content, revalidation behavior, ISR + PPR, per-request cache reasons (cacheReason) and…
  host: grok-build
