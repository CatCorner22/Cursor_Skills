...
user-invocable: true
when-to-use: 'Trigger on: vercel connect; slack token; slack bot token; post to slack; send slack message; github oauth token; linear oauth; oauth token for; third-party token; connect to slack. Also /vercel-connect.'
argument-hint: /vercel-connect task
paths:
- agent/connections/**
- agent/channels/**
...
metadata:
  author: CatCorner22
  short-description: Vercel Connect expert guidance — securely obtain scoped OAuth tokens for third-party services (Slack, GitHub, MCP servers, OAuth,…
  host: grok-build
