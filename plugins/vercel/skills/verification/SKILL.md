...
user-invocable: true
when-to-use: 'Trigger on: verify the flow; verify everything works; test the whole thing; does it actually work; check end to end; end to end test; why isn''t it working right; why doesn''t it work; it''s not working correctly; something''s off. Also /verification.'
argument-hint: /verification task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: 'Full-story verification — infers what the user is building, then verifies the complete flow end-to-end: browser → API → data → response'
  host: grok-build
