...
user-invocable: true
when-to-use: 'Trigger on: backend on vercel; vercel backend; build a backend; create a backend; deploy my backend; backend architecture; api; database; service; workflow. Also /create-a-backend.'
argument-hint: /create-a-backend task
paths:
- Dockerfile.vercel
...
metadata:
  author: CatCorner22
  short-description: Backend architecture guidance
  host: grok-build
