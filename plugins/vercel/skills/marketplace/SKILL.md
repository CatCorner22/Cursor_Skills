...
user-invocable: true
when-to-use: 'Trigger on: vercel integration; marketplace; add stripe; add shopify; provision database. Also /marketplace.'
argument-hint: /marketplace task
paths:
- integration.json
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Vercel Marketplace expert guidance — discovering, installing, and managing third-party integrations via the `vercel integration` CLI
  host: grok-build
