...
user-invocable: true
when-to-use: 'Trigger on: vercel firewall; vercel waf; attack mode; ddos protection; ip block; managed ruleset; bot protection; system bypass; rate limit rule. Also /vercel-firewall.'
argument-hint: /vercel-firewall task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Vercel Firewall expert guidance — automatic DDoS mitigation, the Vercel WAF (custom rules, IP blocking, managed rulesets, rate limiting),…
  host: grok-build
