...
user-invocable: true
when-to-use: Use when intercepting requests at the platform level.
argument-hint: /routing-middleware task
paths:
- middleware.ts
- middleware.js
- middleware.mts
...
metadata:
  author: CatCorner22
  short-description: Vercel Routing Middleware guidance — request interception before cache, rewrites, redirects, personalization
  host: grok-build
...
| **Next.js 16 Proxy** | `proxy.ts` (root, or `src/proxy.ts` if using `--src-dir`) | Node.js only | Next.js 16+ only | Network-boundary proxy needing full Node APIs. Not a substitute for auth checks at the data layer — defense-in-depth only. |
