...
user-invocable: true
when-to-use: 'Use when building or editing the React/Spectrum front-end SPA of a Workfront App Builder extension. Reach for this whenever the user is: registering or changing extension points in `ExtensionRegistration` — a Main Menu button, a left-panel (`secondaryNav`) item for a specific Workfront object type (Project, Task, Issue, Portfolio, Program), or a custom-form widget with specific height and width; a'
argument-hint: /workfront-ui-extension task
license: Apache-2.0
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Use when building or editing the React/Spectrum front-end SPA of a Workfront App Builder extension
  host: grok-build
