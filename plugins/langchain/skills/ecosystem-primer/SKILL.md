...
user-invocable: true
when-to-use: 'Use when the user names LangChain, LangGraph, LangSmith, or Deep Agents, or when the repo already depends on `langchain*` / `langgraph*` / `deepagents` / `@langchain/*` and the layer choice is still open. Scope boundary: an agent-building request that has not chosen a framework is not this pack''s — TypeScript/Vercel agents belong to `build-agents`/`eve`, and Python agents on another framework to `'
argument-hint: /ecosystem-primer task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: 'Router for the LangChain ecosystem: choose between LangChain, LangGraph, and Deep Agents, set the LANGSMITH_* env vars, find the right…'
  host: grok-build
...
This is your **layer**. Step 4 names the layer-specific skill that carries the current API — read it before writing agent code, because this primer does not carry that API.
...
If the user only wants a minimal local working agent (new project, stub tool, provider key), follow the live quickstart page for the layer from Step 1 — swap `python` → `javascript` for TypeScript:

| Layer | Quickstart page |
|---|---|
| LangChain | `/oss/python/langchain/quickstart` |
| LangGraph | `/oss/python/langgraph/quickstart` — prefer the Graph API path over the Functional API |
| Deep Agents | `/oss/python/deepagents/quickstart` |

Scaffold into a **new** directory rather than the open project, keep the provider API key in a gitignored `.env`, and ask which `provider:model` string to use instead of hardcoding the docs' default.

Otherwise read the skill below that matches your layer. The layer-specific skill carries the current API; the primer alone does not.
...
- **`langchain-middleware`** — middleware hooks, tool-call approval, structured output with Pydantic/Zod

### LangGraph

- **`langgraph-fundamentals`** — writing the graph itself
...
- **`langgraph-cli`** — `langgraph dev`/`build`/`up`/`deploy` and `langgraph.json`

### Deep Agents

- **`deep-agents-core`** — the harness itself: `create_deep_agent`, middleware selection, configuration boundaries
- **`deep-agents-orchestration`** — subagent delegation and task planning

Deep Agents memory backends (`StateBackend` / `StoreBackend` / `CompositeBackend`) have no skill in this pack — read `/oss/python/deepagents/` per Step 3.

### Cross-cutting

- **`langchain-dependencies`** — package versions, installs, or dependency management
- **`langsmith-online-eval-engineering`** — online evaluators over LangSmith traces once something is running
