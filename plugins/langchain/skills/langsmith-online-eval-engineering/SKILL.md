...
user-invocable: true
when-to-use: Use when the task matches this skill. Also /langsmith-online-eval-engineering.
argument-hint: /langsmith-online-eval-engineering task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: 'Create LangSmith online evaluators one at a time: inspect recent traces in a tracing project, interview the user, propose grounded…'
  host: grok-build
