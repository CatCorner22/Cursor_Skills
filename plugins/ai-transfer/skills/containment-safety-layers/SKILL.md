...
user-invocable: true
when-to-use: 'Trigger on: containment safety; independent safety layers; BSL layers. Also /containment-safety-layers.'
argument-hint: /containment-safety-layers task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Require independent safety layers (pattern, keyword, policy) that can each fail closed
  host: grok-build
