...
user-invocable: true
when-to-use: 'Trigger on: load bearing; protected claims; structural sentences. Also /load-bearing-structure.'
argument-hint: /load-bearing-structure task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Mark load-bearing claims (therefore/must/key/critical) and forbid polish passes from deleting or softening them
  host: grok-build
