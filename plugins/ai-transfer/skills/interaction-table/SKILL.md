...
user-invocable: true
when-to-use: 'Trigger on: interaction table; plugin conflict; skill pair contraindication. Also /interaction-table.'
argument-hint: /interaction-table task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Look up known skill/plugin pairs that fight (context strip vs inject, length vs rhythm, duck vs dual-axis, rewrite vs lock) before loading…
  host: grok-build
