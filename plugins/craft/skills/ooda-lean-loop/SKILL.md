...
user-invocable: true
when-to-use: 'Trigger on: OODA; OODA loop; what should I do next; prioritize; kaizen; small batch; stop the line; too much WIP; lean loop; andon. Also /ooda-lean-loop.'
argument-hint: /ooda-lean-loop task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: 'Run fused OODA and lean loops: observe raw signal, orient with current-state map, decide one small batch, act with standard work, andon on…'
  host: grok-build
