...
user-invocable: true
when-to-use: 'Use when structuring a feature, splitting services, reviewing coupling, or when changes in one file break unrelated areas. Scope boundary: line-level clarity → `clean-minimal-code`; defining what done looks like → `deliverable-first`; running tests while editing → `real-time-testing`.'
argument-hint: /stable-architecture task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: 'Design code that avoids houses of cards: clear module boundaries, dependency direction, fail-fast invariants, and replaceable adapters'
  host: grok-build
