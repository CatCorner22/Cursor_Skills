...
user-invocable: true
when-to-use: Use when the task matches this skill. Also /huggingface-local-models.
argument-hint: /huggingface-local-models task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Use to select models to run locally with llama.cpp and GGUF on CPU, Mac Metal, CUDA, or ROCm
  host: grok-build
