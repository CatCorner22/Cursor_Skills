...
user-invocable: true
when-to-use: Use when the task matches this skill. Also /hf-cloud-python-env-setup.
argument-hint: /hf-cloud-python-env-setup task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Set up an isolated Python environment for SageMaker / AWS work, with the right Python version and current boto3
  host: grok-build
