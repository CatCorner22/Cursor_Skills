...
user-invocable: true
when-to-use: Use when writing or reviewing code that uses `@spaces.GPU`, configuring `python_version` or `requirements.txt` for a ZeroGPU Space, or handling ZeroGPU-specific code constraints — pickle-based process isolation, `gr.State` semantics across the worker boundary, no `torch.compile` (use AoTI instead), CUDA wheel-only **builds** (no `nvcc` at build; runtime has `nvcc` for AoTI), large vs xlarge sizing
argument-hint: /huggingface-zerogpu task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: AI demos and GPU compute with Gradio Spaces and Hugging Face Spaces ZeroGPU
  host: grok-build
