...
user-invocable: true
when-to-use: Use when the task matches this skill. Also /huggingface-tool-builder.
argument-hint: /huggingface-tool-builder task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Use this skill when the user wants to build tool/scripts or achieve a task where using data from the Hugging Face API would help
  host: grok-build
