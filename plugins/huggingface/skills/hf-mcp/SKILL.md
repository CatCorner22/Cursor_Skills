...
user-invocable: true
when-to-use: Use when the task matches this skill. Also /hf-mcp.
argument-hint: /hf-mcp task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Use Hugging Face Hub via MCP server tools when the Huggingface-skills (or equivalent) MCP namespace is connected
  host: grok-build
