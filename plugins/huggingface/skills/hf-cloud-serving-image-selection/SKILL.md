...
user-invocable: true
when-to-use: Use when the task matches this skill. Also /hf-cloud-serving-image-selection.
argument-hint: /hf-cloud-serving-image-selection task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Pick the right serving container for a SageMaker model deployment and find its current image URI
  host: grok-build
