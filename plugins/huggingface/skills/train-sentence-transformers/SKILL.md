...
user-invocable: true
when-to-use: Use when the task matches this skill. Also /train-sentence-transformers.
argument-hint: /train-sentence-transformers task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Train or fine-tune sentence-transformers models across `SentenceTransformer` (bi-encoder; dense or static embedding model; for retrieval,…
  host: grok-build
