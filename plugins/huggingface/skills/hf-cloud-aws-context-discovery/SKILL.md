...
user-invocable: true
when-to-use: Use when the task matches this skill. Also /hf-cloud-aws-context-discovery.
argument-hint: /hf-cloud-aws-context-discovery task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Discover the user's local AWS context (active profile, region, account ID, caller identity) at the start of any AWS task
  host: grok-build
