...
user-invocable: true
when-to-use: Use when the task matches this skill. Also /hf-cloud-sagemaker-production-defaults.
argument-hint: /hf-cloud-sagemaker-production-defaults task
compatibility: Grok Build (CLI, TUI, IDE). Also readable as Agent Skills / AGENTS.md.
metadata:
  author: CatCorner22
  short-description: Create a SageMaker endpoint (real-time, real-time scale-to-zero, or async) with autoscaling, CloudWatch alarms, and tagging enabled by…
  host: grok-build
