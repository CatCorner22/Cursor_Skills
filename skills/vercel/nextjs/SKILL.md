...
validate:
  ...
  -
    ...
    message: 'NextAuth/Auth.js detected — loading the auth skill, which covers Auth.js v5 setup and the managed-provider options. Keep the user''s chosen auth library unless they ask otherwise.'
    severity: recommended
    upgradeToSkill: auth
    upgradeWhy: 'The auth skill covers Auth.js v5 configuration and the managed-provider options when the user asks for them.'
...
chainTo:
  ...
  -
    pattern: 'from\s+[''""](next-auth|@auth/core)[''""]|NextAuth\(|getServerSession\('
    targetSkill: auth
    message: 'NextAuth/Auth.js detected — loading the auth skill (covers Auth.js v5 and managed providers).'
  -
    pattern: 'NextApiRequest|NextApiResponse|export\s+default\s+function\s+handler'
...
See [file-conventions.md](./references/file-conventions.md) and [app-router-files.md](./references/app-router-files.md) for:
